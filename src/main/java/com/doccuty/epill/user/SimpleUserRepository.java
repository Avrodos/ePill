...
public interface SimpleUserRepository<T extends SimpleUser> extends CrudRepository<T, Long> {
    ...
    @Query("SELECT new SimpleUser(u.id, u.firstname, u.lastname) FROM SimpleUser u WHERE u.gid LIKE :gid")
    T findByGID(@Param(value = "gid") String gid);

    @Query("SELECT new SimpleUser(u.id, u.firstname, u.lastname) FROM SimpleUser u WHERE u.a7id LIKE :a7id")
