...
public interface UserRepository extends SimpleUserRepository<User> {
    ...
    @Query("SELECT u FROM User u WHERE u.gid = :gid")
    User findUserByGID(@Param(value = "gid") String gid);

    @Query("SELECT u FROM User u WHERE u.a7id = :a7id")
    User findUserByA7Id(@Param(value = "a7id") String a7id);
}