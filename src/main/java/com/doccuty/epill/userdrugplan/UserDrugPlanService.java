...
public class UserDrugPlanService {
	...
	/**
	 * check if plan must be recalculated: future - always past - never today - only
	 * if new drugs must be taken -
	 * 
	 ...
	 * @return
	 */
	private UserDrugPlanItemViewModel mapUserDrugPlanToView(List<UserDrugPlanItem> plannedItemsForHour,
			User currentUser) {
		...
		for (int i = 0; i < plannedItemsForHour.size(); i++) {
			if (plannedItemsForHour.get(i).getDrug() != null
		...
		if (plannedItemsForHour.get(0).getId() <= 0) {
			...
			for (UserDrugPlanItem item : plannedItemsForHour) {
				...
				TailoredText tt = this.tailoringService.getTailoredMinimumSummaryByDrugAndUser(item.getDrug(),
						currentUser);
				if (tt != null) {
	...
	private boolean getMealtime(UserDrugPlanItem item) {
		final User currentUser = userService.findUserById(userService.getCurrentUser().getId());
		int hours = DateUtils.getHours(item.getDateTimePlanned());
		if ((hours == currentUser.getBreakfastTime()) || (hours == currentUser.getDinnerTime())
				|| (hours == currentUser.getLunchTime())) {
			return true;
		} else {
			return false;
		}
...
//				private final boolean hasInteractions;
	...
	/**
	 * compute percentage using halftime period and relative hour percentage = 100 *
	 * (0.5) ^ ( halfTimePeriod / relativeHour)
	 * 
	...
	/**
	 * get User drug plan items between two dates (only for planned intake
	 * timestamps)
	 * 
	 * @param dateFrom
	 * @param dateTo
	...
	public void recalculateDrugPlanAfterIntakeChange(long userDrugPlanItemId, boolean isTaken, Integer intakeHour) {
		...
		LOG.info("item: drugid={}, userid={}", item.getDrug().getId(), item.getUser().getId());
		...
		final UserDrugPlanCalculator calculator = new UserDrugPlanCalculator(currentUser,
				drugService.findUserDrugsTaking(currentUser));
		final List<UserDrugPlanItem> drugsPlannedforRestOfDay = getUserDrugPlansByUserIdAndDate(hourBeforeRestOfDay,
				endOfDay);
	...
	public void setDrugTaken(long userDrugPlanItemId, boolean isTaken, Integer intakeHour) {
		LOG.info("set drug taken {} for userDrugPlanItemId {}, intake hour = {}", userDrugPlanItemId, isTaken,
				intakeHour);
		UserDrugPlanItem item = userDrugPlanRepository.getOne(userDrugPlanItemId);
		...
		LOG.info("updated drugTaken = {} for userDrugPlanItemId {}, intakeTime = {}", userDrugPlanItemId,
				item.getDrugTaken(), item.getDateTimeIntake());
	}
	...
	public void saveUserPrescription(UserPrescriptionRequestParameter requestParam) {
		LOG.info("save user prescription for drug_id = {}, period in days={}", requestParam.getDrugId(),
				requestParam.getPeriodInDays());
		final User currentUser = userService.findUserById(userService.getCurrentUser().getId());
		Drug drug = drugService.findDrugById(requestParam.getDrugId());
		if (drug != null) {
			List<UserPrescription> prescriptions = userPrescriptionRepo
					.findPrescriptions(userService.getCurrentUser().getId(), requestParam.getDrugId());
			deletePrescriptions(prescriptions);

			prescriptions = userPrescriptionRepo.findPrescriptions(userService.getCurrentUser().getId(),
					requestParam.getDrugId());
			if (prescriptions.isEmpty()) {
				...
				LOG.info("user prescription saved: id = {}, items = {}", upSaved.getId(),
						upSaved.getUserPrescriptionItems().size());
			} else {
	...
	public UserPrescriptionRequestParameter getUserPrescription(long drugId) {
		final User currentUser = userService.findUserById(userService.getCurrentUser().getId());
		List<UserPrescription> prescriptions = userPrescriptionRepo
				.findPrescriptions(userService.getCurrentUser().getId(), drugId);
		if (!prescriptions.isEmpty()) {
